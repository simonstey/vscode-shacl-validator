...
function renderReport(report) {
  ...
  dataFileEl.textContent = report.dataDocumentUri.substring(
    report.dataDocumentUri.lastIndexOf("/") + 1
  );
  shapesFileEl.textContent = report.shapesDocumentUri.substring(
    report.shapesDocumentUri.lastIndexOf("/") + 1
  );
  ...
  if (report.conforms || !report.results || report.results.length === 0) {
    ...
    report.results.forEach((result, index) => {
      ...
      messageCell.textContent = Array.isArray(result.message)
        ? result.message.join("; ")
        : result.message;
      ...
      if (result.focusNode && result.focusNode.value) {
        const link = createJumpLink(
          ...
          report.dataDocumentUri,
          result.focusNode.termType
        );
      ...
      row.insertCell().textContent = termToString(
...
function termToString(term, simplify = false) {
  ...
  if (term.termType === "Literal") {
    ...
    if (term.language) displayValue += `@${term.language}`;
...
function createJumpLink(termString, targetUri, termType) {
  ...
  link.textContent = termToString(
    { value: termString, termType: termType },
    true
  ); // Simplify display
  link.title = `Jump to: ${termString} in ${targetUri.substring(
    targetUri.lastIndexOf("/") + 1
  )}`;
