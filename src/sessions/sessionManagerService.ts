...
export class SessionManagerService implements vscode.Disposable {
  ...
  private _onDidChangeSessions: vscode.EventEmitter<
    ValidationSession | undefined
  > = new vscode.EventEmitter<ValidationSession | undefined>();
  public readonly onDidChangeSessions: vscode.Event<
    ValidationSession | undefined
  ...
  private async loadSessions() {
    const storedSessions = this.context.workspaceState.get<any[]>(
      this.storageKey,
      []
    );
  ...
  public async createSession(
    ...
    const sessionName =
      name ||
      `Session ${this.sessions.size + 1} (${path.basename(
        dataGraphUri.fsPath
      )} vs ${path.basename(shapesGraphUri.fsPath)})`;
  ...
  public async updateSessionReport(
    id: string,
    report: WebviewValidationReport
  ): Promise<void> {
    const session = this.sessions.get(id);
    if (session) {
  ...
  public async updateSessionDataGraph(
    sessionId: string,
    newDataGraphUri: vscode.Uri
  ): Promise<ValidationSession | undefined> {
    const session = this.sessions.get(sessionId);
    if (session) {
      session.dataGraphUri = newDataGraphUri;
      session.dataGraphFileName = path.basename(newDataGraphUri.fsPath);
      session.lastValidationReport = undefined; // Clear last report as data changed
  ...
  public async updateSessionShapesGraph(
    sessionId: string,
    newShapesGraphUri: vscode.Uri
  ): Promise<ValidationSession | undefined> {
    const session = this.sessions.get(sessionId);
    if (session) {
      session.shapesGraphUri = newShapesGraphUri;
      session.shapesGraphFileName = path.basename(newShapesGraphUri.fsPath);
      session.lastValidationReport = undefined; // Clear last report as shapes changed
      this.sessions.set(sessionId, session);
      await this.saveSessions();
      this._onDidChangeSessions.fire(session);
      return session;
