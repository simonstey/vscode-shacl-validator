...
export class ShaclCodeLensProvider implements vscode.CodeLensProvider {
  ...
  constructor(
    ...
    vscode.workspace.onDidChangeConfiguration((e) => {
      if (
        e.affectsConfiguration("shaclValidator.enableCodeLens") ||
  ...
  public provideCodeLenses(
    ...
    const shapesContext = this.rdfDocumentManager.getDocumentContext(
      document.uri
    );
    ...
    const turtleEditors = editors.filter(
      (e) =>
        e.document.languageId === "turtle" &&
    ...
    const dataContext = this.rdfDocumentManager.getDocumentContext(
      dataDocument.uri
    );
    ...
    const nodeShapeQuads = shapesStore.getQuads(
      null,
      namedNode(RDF + "type"),
      namedNode(SH + "NodeShape"),
      null
    ...
    for (const shapeQuad of nodeShapeQuads) {
      ...
      console.log(
        `  - Found ${focusNodes.length} focus nodes for shape ${shapeIRI}`
      ); // Find shape declaration position in document
      const shapePosition = this.findShapePosition(document, shapeIRI);
      if (shapePosition) {
        console.log(
          `  - Found position for shape ${shapeIRI}: line ${
            shapePosition.start.line + 1
          }`
        );
  ...
  private extractShapeTargets(
    ...
    const targetClassQuads = store.getQuads(
      namedNode(shapeIRI),
    ...
    const targetNodeQuads = store.getQuads(
      namedNode(shapeIRI),
    ...
    const targetSubjectsOfQuads = store.getQuads(
      namedNode(shapeIRI),
    ...
    const targetObjectsOfQuads = store.getQuads(
      namedNode(shapeIRI),
    ...
    if (targets.length === 0) {
      const classTypeQuads = store.getQuads(
        namedNode(shapeIRI),
        namedNode(RDF + "type"),
    ...
    for (const target of targets) {
      switch (target.type) {
        ...
        case ShaclTargetType.IMPLICIT_CLASS:
          ...
          const classInstances = this.findInstancesOfClass(
            dataStore,
            target.value
          );
        ...
        case ShaclTargetType.SUBJECTS_OF:
          ...
          const subjects = this.findSubjectsOfPredicate(
            dataStore,
            target.value
          );
   ...
   */
  private findSubjectsOfPredicate(
    dataStore: N3.Store,
    predicate: string
  ): string[] {
   ...
   */
  private findObjectsOfPredicate(
    dataStore: N3.Store,
    predicate: string
  ): string[] {
  ...
  private findShapePosition(
    ...
    specificPatterns.push(
      `<${escapedShapeIRI}>\\s+(?:<${RDF}type>|a)\\s+(?:<${SH}NodeShape>|sh:NodeShape)`
    );
    ...
    for (const [prefix, uri] of Object.entries(prefixes)) {
      if (shapeIRI.startsWith(uri)) {
        ...
        const escapedPrefixedName = prefixedName.replace(
          /[.*+?^${}()|[\]\\]/g,
          "\\$&"
        );
        specificPatterns.push(
          `${escapedPrefixedName}\\s+(?:<${RDF}type>|a|rdf:type)\\s+(?:<${SH}NodeShape>|sh:NodeShape)`
        );
    ...
    if (localName) {
      ...
      specificPatterns.push(
        `\\b${escapedLocalName}\\b\\s+(?:<${RDF}type>|a|rdf:type)\\s+(?:<${SH}NodeShape>|sh:NodeShape)`
      );
      specificPatterns.push(
    ...
    for (const patternStr of specificPatterns) {
      try {
        ...
        while ((match = pattern.exec(text)) !== null) {
          ...
          for (const [prefix, uri] of Object.entries(prefixes)) {
            if (shapeIRI.startsWith(uri)) {
              const prefixedName = `${prefix}:${shapeIRI.substring(
                uri.length
              )}`;
    ...
    const genericPatterns = [
      // Look for directly preceding IRI/prefix declarations for shapes
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (
        ...
        console.log(
          `Found shape ${shapeIRI} on line ${i + 1} with full IRI pattern`
        );
      ...
      for (const prefixedForm of possiblePrefixedForms) {
        if (
          ...
          console.log(
            `Found shape ${shapeIRI} on line ${
              i + 1
            } with prefixed form ${prefixedForm}`
          );
          return new vscode.Range(
            new vscode.Position(i, 0),
            new vscode.Position(i, line.length)
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.includes(`<${shapeIRI}>`)) {
        ...
        for (let j = i; j < Math.min(i + 5, lines.length); j++) {
          if (
            lines[j].includes("sh:NodeShape") ||
            lines[j].includes(`<${SH}NodeShape>`)
          ) {
            console.log(
              `Found shape ${shapeIRI} with lookahead on lines ${i + 1}-${
                j + 1
              }`
            );
            return new vscode.Range(
        ...
        if (prefixPattern.test(line)) {
          ...
          for (let j = i; j < Math.min(i + 5, lines.length); j++) {
            if (
              ...
              console.log(
                `Found shape ${shapeIRI} with prefixed form lookahead on lines ${
                  i + 1
                }-${j + 1}`
              );
              return new vscode.Range(
        ...
        if (localPattern.test(line)) {
          ...
          for (let j = i; j < Math.min(i + 5, lines.length); j++) {
            if (
              ...
              console.log(
                `Found shape ${shapeIRI} with local name lookahead on lines ${
                  i + 1
                }-${j + 1}`
              );
              return new vscode.Range(
