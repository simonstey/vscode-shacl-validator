...
export function activate(context: vscode.ExtensionContext) {
  console.log(
    'Congratulations, your extension "vscode-shacl-validator" is now active!'
  );
  ...
  const sessionsView = vscode.window.createTreeView('shaclPlayground', { // 'shaclPlayground' is the ID from package.json
    treeDataProvider: sessionsTreeDataProvider
  });
  context.subscriptions.push(sessionsView);

  validationResultsViewProvider = new ValidationResultsViewProvider(context.extensionUri, highlightingService, rdfDocumentManager);
  ...
  context.subscriptions.push(
    vscode.languages.registerCodeLensProvider(
      { language: 'turtle', scheme: '*' },
      shaclCodeLensProvider
    )
  );
  ...
  context.subscriptions.push(vscode.commands.registerCommand(
    "vscode-shacl-validator.validateActiveDocument",
    () => shaclService.validateActiveDocument()
  ));
  context.subscriptions.push(vscode.commands.registerCommand(
    "vscode-shacl-validator.validateDocument",
    (uri?: vscode.Uri) => { /* ... existing logic ... */ }
  ));

  // Highlighting Commands
  context.subscriptions.push(vscode.commands.registerCommand(
    "vscode-shacl-validator.highlightSelection",
    () => highlightingService.highlightSelection()
  ));
  context.subscriptions.push(vscode.commands.registerCommand(
    "vscode-shacl-validator.clearHighlights",
    () => highlightingService.clearHighlights()
  ));

  // Session Commands
  context.subscriptions.push(vscode.commands.registerCommand('vscode-shacl-validator.createSession', async () => {
    const dataGraphUri = await vscode.window.showOpenDialog({
      ...
      filters: { 'RDF Files': ['ttl', 'rdf', 'shacl', 'shc', 'n3', 'jsonld', 'nt'], 'All files': ['*'] },
      defaultUri: vscode.workspace.workspaceFolders?.[0]?.uri
    });
    if (!dataGraphUri || dataGraphUri.length === 0) { return; };
    ...
    const shapesGraphUri = await vscode.window.showOpenDialog({
      ...
      filters: { 'RDF Files': ['ttl', 'rdf', 'shacl', 'shc', 'n3', 'jsonld', 'nt'], 'All files': ['*'] },
      defaultUri: dataGraphUri[0] ? vscode.Uri.file(path.dirname(dataGraphUri[0].fsPath)) : vscode.workspace.workspaceFolders?.[0]?.uri
    });
    if (!shapesGraphUri || shapesGraphUri.length === 0) { return; };

    const sessionName = await vscode.window.showInputBox({
      prompt: "Enter a name for this validation session (optional)",
      placeHolder: `Session: ${path.basename(dataGraphUri[0].fsPath)} vs ${path.basename(shapesGraphUri[0].fsPath)}`
    });
  ...
  context.subscriptions.push(vscode.commands.registerCommand('vscode-shacl-validator.runSessionValidation', async (sessionIdOrItem: string | ValidationSessionItem) => {
    let sessionId: string;
    if (typeof sessionIdOrItem === 'string') {
      sessionId = sessionIdOrItem;
    } else if (sessionIdOrItem instanceof ValidationSessionItem && sessionIdOrItem.itemType === 'sessionRoot') {
  ...
  context.subscriptions.push(vscode.commands.registerCommand('vscode-shacl-validator.viewSessionReport', (sessionIdOrItem: string | ValidationSessionItem) => {
    let sessionId: string;
    if (typeof sessionIdOrItem === 'string') {
      sessionId = sessionIdOrItem;
    } else if (sessionIdOrItem instanceof ValidationSessionItem) { // Could be any item type if command is on sessionRoot
      sessionId = sessionIdOrItem.session.id;
    } else {
    ...
    if (session?.lastValidationReport) {
      ...
      vscode.window.showInformationMessage(`No report for session '${session.name}'. Run validation first.`);
  ...
  context.subscriptions.push(vscode.commands.registerCommand('vscode-shacl-validator.deleteSession', async (item: ValidationSessionItem) => {
  ...
  context.subscriptions.push(vscode.commands.registerCommand('vscode-shacl-validator.renameSession', async (item: ValidationSessionItem) => {
    if (item && item.session && item.itemType === 'sessionRoot') {
  ...
  context.subscriptions.push(vscode.commands.registerCommand('vscode-shacl-validator.openSessionFile', async (fileUri: vscode.Uri) => {
    if (fileUri) {
      try {
  ...
  context.subscriptions.push(vscode.commands.registerCommand('vscode-shacl-validator.refreshSessions', () => {
    sessionsTreeDataProvider.refresh();
  }));

  // CodeLens commands for focus nodes
  context.subscriptions.push(vscode.commands.registerCommand('vscode-shacl-validator.highlightFocusNodes',
    async (focusNodes: string[], dataDocumentUri: vscode.Uri) => {
      try {
          ...
          for (const pattern of searchPatterns) {
            ...
            try {
              ...
              while ((match = regex.exec(text)) !== null) {
                ...
                const startPos = dataEditor.document.positionAt(match.index + (match[0].length - matchedText.length));
                const endPos = dataEditor.document.positionAt(match.index + match[0].length - 1); // -1 to exclude the trailing character
        ...
        if (!foundAny) {
          vscode.window.showInformationMessage('No focus node patterns found in the document. They might be using different formats than expected.');
        } else {
  ...
  context.subscriptions.push(vscode.commands.registerCommand('vscode-shacl-validator.selectDataDocumentForFocusNodes',
    async (shapeIRI: string, shapesDocUri: vscode.Uri) => {
      ...
      try {
        ...
        vscode.commands.executeCommand('vscode-shacl-validator.highlightFocusNodes', focusNodes, dataFileUri[0]);
      } catch (error) {
  ...
  context.subscriptions.push(vscode.commands.registerCommand(
    'vscode-shacl-validator.internal.showDocumentInColumn',
    showDocumentInColumn
  ));
